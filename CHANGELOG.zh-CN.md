...
- 模型写给子代理的任务文本不再作为用户提示词上传，也不会被当成控制指令解析；子代理进入空闲不再产生 Stop 事件。
- 在本地被丢弃的事件（`tracekin off`、`tracekin_deny`、清除、过期清理、被接收服务拒绝）现在会在数据库文件里被覆写（`PRAGMA secure_delete`）；此前它们的明文仍留在文件的空闲页里。这只涉及本机文件，被丢弃的事件从未上传。
