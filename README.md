...
请先阅读 `plugins/tracekin/README.md`。版本 `0.6.0` 同时支持 Codex、Claude Code 和 Cursor（Cursor 用 `python3 plugins/tracekin/scripts/tracekin.py install-cursor` 注册 hooks 与 MCP）：安装后默认开启 Tracekin Cloud 同步，首次 `SessionStart` 自动绑定当前项目，旧数据库自动迁移为默认开启，只有显式 `tracekin_deny` 会保持全局关闭；两种 harness 共用 `~/.tracekin` 数据目录（自动采用旧的 `~/.codex/tracekin`），`tracekin_status` 为纯读操作并返回 `data_dir`，提供 Dashboard 和 `tracekin off/on/status` 当前会话控制。
...
升级后应显示版本 `0.6.0+build.<cachebuster>`；重启 Codex Desktop 后，在 Hooks 页面重新审阅当前定义，并从项目目录新建会话让 `SessionStart` 完成迁移和项目绑定。
