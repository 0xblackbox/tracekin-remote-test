...
请先阅读 `plugins/tracekin/README.md`。版本 `0.4.4` 安装后默认开启 Tracekin Cloud 同步，首次 `SessionStart` 自动绑定当前项目，旧数据库（pending / 缺少决策 / 仅 false）自动迁移为默认开启，只有显式 `tracekin_deny` 会保持全局关闭；hooks 与 MCP 统一使用 `~/.codex/tracekin` 数据目录，`tracekin_status` 为纯读操作并返回 `data_dir`，提供 Dashboard 和 `tracekin off/on/status` 当前会话控制。
...
升级后应显示版本 `0.4.4+codex.<cachebuster>`；重启 Codex Desktop 后，在 Hooks 页面重新审阅当前定义，并从项目目录新建会话让 `SessionStart` 完成迁移和项目绑定。
