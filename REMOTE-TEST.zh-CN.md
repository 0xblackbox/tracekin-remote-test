...
已装旧版时先升级：Codex 用 `marketplace upgrade` → `remove` → `add`，Claude Code 用 `claude plugin update`，Cursor 与 OpenCode 在 checkout 里 `git pull`，Gemini CLI 用 `gemini extensions update tracekin`。版本应显示 `0.8.0+build.<时间戳>`。
