...
已装旧版时先升级：Codex 用 `marketplace upgrade` → `remove` → `add`，Claude Code 用 `claude plugin update`，Cursor 与 OpenCode 在 checkout 里 `git pull`，Gemini CLI 用 `gemini extensions update tracekin`。版本应显示 `0.8.3+build.<时间戳>`。
...
插件目录：Codex 在 `~/.codex/plugins/cache/<marketplace>/tracekin/<version>/`，Claude Code 在 `~/.claude/plugins/cache/...`，Gemini CLI 在 `~/.gemini/extensions/tracekin/plugins/tracekin/`，Cursor 与 OpenCode 就是 checkout。这条是纯读命令，等价于 MCP `tracekin_status`。
