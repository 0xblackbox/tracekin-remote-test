...
Upgrade first if an older version is installed: Codex uses `marketplace upgrade` → `remove` → `add`, Claude Code uses `claude plugin update`, Cursor and OpenCode use `git pull` in the checkout, Gemini CLI uses `gemini extensions update tracekin`. The version should read `0.8.1+build.<timestamp>`.
