...
请先阅读 `plugins/tracekin/README.md` 中的“跨电脑完整 Hooks/MCP 测试”章节。版本 `0.4.6` 安装后默认开启同步，首次 `SessionStart` 自动绑定当前项目，旧数据库自动迁移为默认开启（显式 `tracekin_deny` 除外），hooks 与 MCP 统一使用 `~/.codex/tracekin` 数据目录，`tracekin_status` 为纯读查询并返回 `data_dir`；支持 `tracekin off/on/status` 当前会话控制，控制提示不会上传。
