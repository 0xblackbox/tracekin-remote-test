...
请先阅读 `plugins/tracekin/README.md` 中的“跨电脑完整 Hooks/MCP 测试”章节。版本 `0.2.2+codex.20260916053911` 支持 `tracekin off/on/status` 当前会话控制；控制提示不会上传。此包不含接收服务 Bearer token。
...
升级后应显示版本 `0.2.2+codex.20260916053911`；重启 Codex Desktop 后，在 Hooks 页面重新审阅当前定义。
